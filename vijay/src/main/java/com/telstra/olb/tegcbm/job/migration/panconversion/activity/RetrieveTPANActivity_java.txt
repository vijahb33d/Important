 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class RetrieveTPANActivity implements Activity{
	...
	private void getContents(String host, String user, String pwd, int port, String location, String filename) throws SFTPConnectionException, ActivityException, CreditCardDataException {
		...
		int countStart = 0;
		int countEnd = getPropertyInt(RECORD_COUNT_LENGTH);
		...
		int datFileStart = countEnd + 10;
		int datFilenameLen = getPropertyInt(DAT_FILENAME_LENGTH);
		String datFileName = ctlContent.substring(datFileStart,datFilenameLen+datFileStart).trim();
